...
**Description:** This tutorial describes the administration of level 3 advanced
authorization rules into **Authzforce**, either directly, or with the help of
the **Keyrock** GUI. The simple verb-resource based permissions are amended to
use XACML and new XACML permissions added to the existing roles. The updated
ruleset is automatically uploaded to **Authzforce** PDP, so that policy
execution points such as the **PEP proxy** are able to apply the latest ruleset.
...
controls accordingly. The [previous tutorial](xacml-access-rules.md) included a
static XACML `<PolicySet>` loaded into **Authzforce**. This component offers
advanced authorization (level 3) access control where every policy decision is
calculated on the fly and new rules can be applied under new circumstances. The
details of the [Authzforce](https://authzforce-ce-fiware.readthedocs.io/) Policy
Decision Point (PDP) were discussed in the
[previous tutorial](xacml-access-rules.md), suffice to say, the **Authzforce**
...
application created in [previous tutorials](xacml-access-rules.md) and secures
access to the context broker behind a
...
[**(Click to EXPAND)**](cmds/administrating-xacml.md#13-request)
...
To see the new `<Rule>`
[**(Click to EXPAND)**](cmds/administrating-xacml.md#update-an-xacml-permission)
...
Alternatively to programatically amend the XACML rules governing a permission,
