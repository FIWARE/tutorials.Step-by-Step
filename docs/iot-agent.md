...
The `iot-agent` container relies on the precence of the Orion Context Broker and
...
In the example the IoT Agent is informed that the `/iot/d` endpoint will be used
...
The `refStore` attribute comes from the `static_attributes` set when the device
was provisioned.
...
Before we wire-up the context broker, we can test that a command can be sent to
