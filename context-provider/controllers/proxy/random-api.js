    ...
    switch (type.toLowerCase()) {
        case 'array':
            ...
            for (let i = 0; i < randy; i++) {
                ret.push(LOREM_IPSUM_WORD_BANK[Math.floor(Math.random() * (LOREM_IPSUM_WORD_BANK.length - 1))]);
            }
            break;
        case 'list':
            ret = [];
...
function getLoremIpsum() {
    ...
    // eslint-disable-next-line id-blacklist
    for (let i = 0; i < randy; i++) {
        let newTxt = LOREM_IPSUM_WORD_BANK[Math.floor(Math.random() * (LOREM_IPSUM_WORD_BANK.length - 1))];
        if (
