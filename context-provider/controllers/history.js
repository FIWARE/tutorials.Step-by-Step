...
const cometUrl =
  (process.env.STH_COMET_SERVICE_URL || 'http://localhost:8666/STH/v1') +
  '/contextEntities/type/';
const crateUrl =
  process.env.CRATE_DB_SERVICE_URL || 'http://localhost:4200/_sql';
...
function cometToTimeSeries(cometResponse, aggMethod, hexColor) {
  ...
  if (
    ...
    cometResponse.contextResponses[0].contextElement.attributes[0].values
      .length > 0
  ) {
    const values =
